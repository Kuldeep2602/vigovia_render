...
export class PDFGeneratorService {
  ...
  async generateItineraryPDF(data, outputPath) {
    ...
    try {
      ...
      console.log('🚀 Launching Puppeteer...');
      console.log('📋 Puppeteer options:', this.getPuppeteerOptions()); // NEW - Debug log
      
      // CHANGED - Use new getPuppeteerOptions method
      browser = await puppeteer.launch(this.getPuppeteerOptions());
      console.log('✅ Browser launched successfully'); // NEW - Success log

      const page = await browser.newPage();
      console.log('✅ New page created'); // NEW - Debug log
      
      // NEW - Set viewport for consistent rendering
      await page.setViewport({ width: 1200, height: 800 });
      
      // CHANGED - Added timeout and debug logs
      console.log('📄 Setting page content...'); // NEW - Debug log
      await page.setContent(html, {
        waitUntil: ['networkidle0', 'domcontentloaded'],
        timeout: 30000 // NEW - Added timeout
      });
      console.log('✅ Page content set successfully'); // NEW - Success log
      ...
      console.error('❌ PDF generation failed:', error.message); // CHANGED - Better error logging
      console.error('❌ Full error stack:', error.stack); // NEW - Full stack trace
      throw new Error(`PDF generation failed: ${error.message}`);
    } finally {
      // NEW - Improved browser cleanup with error handling
      if (browser) {
        try {
          await browser.close();
  ...
  prepareTemplateData(data) {
    ...
    // Format dates
    const formatDate = (date) => moment(date).format('DD/MM/YYYY');
    const formatDateLong = (date) => moment(date).format('Do MMMR');
